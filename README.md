## Hi 

# 👋 Hi, I'm MAHMOUD SHAHBA

🎯 **Data Engineer with a strong mathematics background**  
I build scalable data pipelines, backend systems, and cloud solutions.  

---

## 🧠 About Me
- 📊 25+ years in Mathematics & Statistics  
- 🔧 Data Engineering & Backend Development  
- ☁️ Cloud & DevOps Enthusiast  
- 🚀 Goal: Build production-grade data systems & transition into ML

---

## ⚡ Tech Stack

### 🐍 Data & Analytics
Python | Pandas | NumPy | Matplotlib | Tableau  

### 🗄️ Databases
PostgreSQL | MongoDB  

### ⚡ Backend
Java (Spring Boot) | Node.js | Express | Microservices | REST APIs  

### 🌐 Frontend
JavaScript | React.js | Next.js | HTML | CSS  

### ☁️ Cloud & DevOps
AWS | Docker | Jenkins | Terraform | Linux | Git  

---

## 📂 Featured Projects

### 🔹 ETL Pipeline Project
- Built a scalable ETL pipeline using Python & PostgreSQL  
- Automated workflow with Docker  
- [Employee-Management-System](https://github.com/MAHMOUD-SHAHBA/Employee_Management_System_Python.git)  

### 🔹 Full-Stack Data App
- Backend: Spring Boot / Node.js  
- Frontend: React / Next.js  
- Deployed on AWS with CI/CD pipelines  
- [In Progress](https://github.com/MAHMOUD-SHAHBA/MAHMOUD-SHAHBA/blob/main/README.md)  

### 🔹 Analytics Dashboard
- Visualized real-world data using Python & Tableau  
- Interactive dashboards with actionable insights  
- [Blinkit-Analysis](https://github.com/MAHMOUD-SHAHBA/Blinkit--Analysis.git)
- [Oizza-Sales-Analysis](https://github.com/MAHMOUD-SHAHBA/Pizza_Sales_Pandas.git)
- [London_Bike](https://github.com/MAHMOUD-SHAHBA/London-Bike-Analysis.git)

---

## 📈 Current Focus
- Building production-grade data pipelines  
- Learning Apache Airflow & Spark  
- Preparing for Machine Learning Engineering  

---

## 🌍 Connect With Me
- 💼 LinkedIn: [Your LinkedIn](#)  
- 📧 Email: ds.m.sh.777@gmail.com  
- 🟢 WhatsApp: +20 100 804 0778 [Contact me ](https://web.whatsapp.com/))

---

## ⚡ Fun Fact
I combine deep mathematical thinking with real-world engineering to build scalable, efficient systems.
